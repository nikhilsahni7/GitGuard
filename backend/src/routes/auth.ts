...
import { generateBiometricToken, verifyBiometricToken } from '../utils/biometricUtils';
 ...
 * @access Private
 */
router.post('/refresh', authenticateJwt, async (req: AuthRequest, res, next) => {
  try {
    if (!req.user) {
 ...
 * @access Private
 */
router.post('/biometric/setup', authenticateJwt, async (req: AuthRequest, res, next) => {
  try {
    if (!req.user) {
      throw new ApiError(401, 'User not authenticated');
    }
...
router.post('/biometric/verify', async (req, res, next) => {
  try {
    ...
    const isValid = await verifyBiometricToken(validatedData.userId, validatedData.token);
 ...
 * @access Private
 */
router.post('/biometric/disable', authenticateJwt, async (req: AuthRequest, res, next) => {
  try {
    if (!req.user) {
      throw new ApiError(401, 'User not authenticated');
    }
